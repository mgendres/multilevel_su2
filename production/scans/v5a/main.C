...
double R(double K)
{
  if (K <= 0.25751180683317054) {
    return 0;
  }
  return K - 0.25751180683317054;
}
      ...
      if ( conf%n_chkpt==0 ) {
        ...
        if (HITS>0) {
          ...
          for(int cool_sweeps=0; cool_sweeps<50; ++cool_sweeps) {
            latticeY.CoolUpdate(Subsets::two_cell_bulk_checker_board);
          }
          ...
          for (int h=0; h<HITS; ++h) {
            ...
            latticeY.HeatBathUpdate(beta);
          }

        }
      } 
      latticeX.HeatBathUpdate(betaX);
