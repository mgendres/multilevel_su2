    ...
    do {
      if ( conf%n_chkpt==0 ) {
        ...
        if (HITS>0) {
          ...
          std::cout << "Copying lattice..." << std::endl;

          if ( !Copy(lattice, latticeX) ) {
            std::cout << "Failed to copy lattice!" << std::endl;
          }

          std::cout << "Decimating lattice..." << std::endl;
          latticeX.Initialize(DISORDERED,Subsets::two_cell_bulk);

          for (int h=0; h<HITS; ++h) {
            std::cout << "Saving observables for configuration/hit : " << conf << "/" << h << std::endl;
            for (int m=0; m<PLAQ_M; ++m)
            for (int n=0; n<PLAQ_M; ++n) {
                plaqs[m][n][h].push_back( 1.0-latticeX.MeanPlaquette(m+1,n+1) );
            }
            latticeX.HeatBathUpdate(beta, Subsets::two_cell_bulk_checker_board);
          }
